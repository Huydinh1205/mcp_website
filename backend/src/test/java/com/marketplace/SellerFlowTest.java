...
class SellerFlowTest {
  ...
  private String firstProductId(String query) throws Exception {
    String s = body(mvc.perform(post("/api/mcp").contentType("application/json")
  ...
  void browserMode_buyerOffer_leavesItWaitingForTheSeller() throws Exception {
    ...
    String state = body(mvc.perform(post("/api/mcp").contentType("application/json")
  ...
  void sellerToolsSeeIncomingOffersScopedToTheSeller() throws Exception {
    ...
    String incoming = body(mvc.perform(post("/api/mcp").contentType("application/json")
        .content("{\"tool\":\"list_incoming_offers\",\"session\":{\"sellerId\":\"S-004\"}}"))
        .andExpect(status().isOk()));
    assertThat(incoming).contains("\"min_price\"").contains("\"auto_accept_price\"");

    // a different seller sees nothing from S-004's product
    String other = body(mvc.perform(post("/api/mcp").contentType("application/json")
        .content("{\"tool\":\"list_incoming_offers\",\"session\":{\"sellerId\":\"S-001\"}}")));
  ...
  void twoSidedConfirm_finalizesOnlyAfterBothClicks() throws Exception {
    ...
    String st = body(mvc.perform(post("/api/mcp").contentType("application/json")
        .content("{\"tool\":\"submit_offer\",\"session\":{\"buyerId\":\"B-002\"},"
    ...
    String sellerResp = body(mvc.perform(post("/api/mcp").contentType("application/json")
        .content("{\"tool\":\"respond_to_offer\",\"session\":{\"sellerId\":\"S-004\"},"
    ...
    String buyerResp = body(mvc.perform(post("/api/mcp").contentType("application/json")
        .content("{\"tool\":\"accept_offer\",\"session\":{\"buyerId\":\"B-002\"},"
    ...
    String buyerConfirm = body(mvc.perform(post("/api/orders/confirm").contentType("application/json")
        .content("{\"confirm_token\":\"" + mintForTest(negId, "buyer") + "\"}"))
        .andExpect(status().isOk()));
    assertThat(buyerConfirm).contains("\"status\":\"pending\"");

    String bothConfirm = body(mvc.perform(post("/api/orders/confirm").contentType("application/json")
